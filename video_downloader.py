...
def download_aslpro(url, dirname, video_id):
    print("Skipping videos from aslpro")
...
def download_nonyt_videos(indexfile, saveto='raw_videos'):
    ...
    for i, entry in enumerate(content, start=0):
        gloss = entry['gloss']
        instances = entry['instances']

        print(f'{i}/{len(content)}')
        ...
        for j, inst in enumerate(instances):
            print(f"Gloss: {gloss}, num of instances: {j}/{len(instances)}")
            video_url = inst['url']
...
def download_yt_videos(indexfile, saveto='raw_videos'):
    ...
    for i, entry in enumerate(content, start=0):
        ...
        print(f'{i}/{len(content)}')
        with open('download_progress_youtube.txt', 'w') as f:
            # Write the text to the file
            f.write(str(i))
        ...
        for j, inst in enumerate(instances):
            ...
            if 'aslpro' in video_url:
                print("Skipping aslpro")
                continue

            if os.path.exists(os.path.join(saveto, video_id + '.mp4')) or os.path.exists(os.path.join(saveto, video_url[-11:] + '.mkv')):
